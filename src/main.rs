...
fn serve_static_file(request: Request, file_path: &str, content_type: &str) -> Result<(), ()> {
    let content_type_header = Header::from_bytes("Content-Type", content_type).expect("That we didn't put any garbage in header");
    let file = File::open(file_path).map_err(|err|{
...
fn serve_api_search(tf_index: &TermFreqIndex, mut request: Request) -> Result<(), ()> {
    ...
    let content_type_header = Header::from_bytes("Content-Type", "application/json").expect("That we didn't put any garbage in the headers");
